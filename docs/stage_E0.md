...
启动介质：SD 卡
存储设备名：暂定 /dev/mmcblk0
分区方案：boot + rootfs_A + rootfs_B + data
文件系统：boot 为 FAT/ext4 待定，rootfs 为 ext4，data 为 ext4
U-Boot env：暂定使用默认环境区，E2 阶段确认 fw_printenv/fw_setenv 可用
```
